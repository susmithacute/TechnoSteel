...
if (isset($_POST['submit_btn'])) {
    ...
    if (!empty($notification_day)) {
        ...
        $sel = $db->selectQuery("SELECT * FROM `sm_notification_days` WHERE `notification_type`='Visa'");
        if (count($sel)) {
            $insert = $db->secure_update("sm_notification_days", $values, "WHERE `notification_type`='Visa'");
        } else {
            $values['notification_type'] = 'Visa';
            $insert = $db->secure_insert("sm_notification_days", $values);
        }
        unset($values);
    } else {
                        ...
                        <form name="step1" id="form1" role="form" method="post">
                            <div class="row">
                                <div class="form-group col-md-6">
                                    ...
                                    <?php
                                    $select_noti_day = $db->selectQuery("SELECT * FROM `sm_notification_days` WHERE `notification_type`='Visa'");
                                    ?>
