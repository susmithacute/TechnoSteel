...
$page = "employee";
$sub = "employee_docs";
$sub1 = "employee_status";
include('./file_parts/header.php');

$empArray = array();
//$values['document_staus_lic']='';
$resFet = $db->selectQuery("SELECT CONCAT( sm_employee.employee_firstname,  ' ', sm_employee.employee_middlename,  ' ', sm_employee.employee_lastname ) AS full_name, sm_employee.employee_employment_id, sm_employee.employee_id,sm_employee.employee_visatype
FROM sm_employee WHERE sm_employee.employee_status =1");
if (count($resFet)) {
	$number=1;
    for ($rC = 0; $rC < count($resFet); $rC++) {
		...
		$values['employee_visatype'] = $resFet[$rC]['employee_visatype'];
		$select_file_data=$db->selectQuery("SELECT * FROM `sm_employee_files` WHERE `employee_id`='$emp_id' AND `file_status`='1'");
		$values['document_staus_lic'] = $values['document_staus_ins'] = $values['document_staus_hea'] = $values['document_staus_pas'] =$values['document_staus_vis'] = $values['document_staus_qat'] = $values['document_staus_res'] = $values['document_staus_pic']=  "Not Uploaded";
		...
		if(count($select_file_data))
		{
			for($i=0; $i < count($select_file_data); $i++){
				...
				if($file_class == 'License'){
					if(!empty($select_file_data[$i]['file_name'])){
						$values['document_staus_lic'] = "Uploaded";
					} else { 
						$values['document_staus_lic'] = "Not Uploaded";}
				} else if($file_class == 'Insurance'){
					if(!empty($select_file_data[$i]['file_name'])){
						$values['document_staus_ins'] = "Uploaded";
					} else { 
						$values['document_staus_ins'] = "Not Uploaded";}
				}else if($file_class == 'Health'){
					if(!empty($select_file_data[$i]['file_name'])){
						$values['document_staus_hea'] = "Uploaded";
					} else { 
						$values['document_staus_hea'] = "Not Uploaded";}
				}else if($file_class == 'Passport'){
					if(!empty($select_file_data[$i]['file_name'])){
						$values['document_staus_pas'] = "Uploaded";
					} else { 
						$values['document_staus_pas'] = "Not Uploaded";}
				}else if($file_class == 'Visa'){
					if(!empty($select_file_data[$i]['file_name'])){
						$values['document_staus_vis'] = "Uploaded";
					} else { 
						$values['document_staus_vis'] = "Not Uploaded";}
				}else if($file_class == 'Qatar'){
					if(!empty($select_file_data[$i]['file_name'])){
						$values['document_staus_qat'] = "Uploaded";
					} else { 
						$values['document_staus_qat'] = "Not Uploaded";}
				}else if($file_class == 'Resume'){
					if(!empty($select_file_data[$i]['file_name'])){
						$values['document_staus_res'] = "Uploaded";
					} else { 
						$values['document_staus_res'] = "Not Uploaded";}
				}
				if($file_title == 'Profile_Picture') {
					if(!empty($select_file_data[$i]['file_name'])){
						$values['document_staus_pic'] = "Uploaded";
					} 
					//else { 
						//$values['document_staus_pic'] = "Not Uploaded";}
				} 
				if($file_class == 'Picture') {
					if(!empty($select_file_data[$i]['file_name'])){
						$values['document_staus_pic'] = "Uploaded";
					}
					//else { 
        ...
        <div class="pageheader">
            ...
            <div class="page-bar">
                ...
                <ul class="page-breadcrumb">
                    ...
                    <li>
                        <a>Employee List-All Employees</a>
                    </li>
                </ul>
                ...
                <div class="col-md-12">
                    ...
                    <section class="tile">
                        ...
                        <div class="tile-header dvd dvd-btm">
                            <h1 class="custom-font"><strong>Employee  List</strong>-All Employees</h1>
                            <ul class="controls">
                                <li class="dropdown">
    ...
    $(window).load(function () {
      ...
      var t=  $('#products-list').DataTable({
            ...
            "columns": [
				...
				{
                    "data": "employee_visatype",
                },	
                {"data": "document_staus_lic"},
                {"data": "document_staus_ins"},
                {"data": "document_staus_hea"},
				{"data": "document_staus_pas"},
				{"data": "document_staus_vis"},
                {"data": "document_staus_qat"},
                {"data": "document_staus_res"},
				{"data": "document_staus_pic"}
                /*{
                    "type": "html",
                    "data": "employee_id",
