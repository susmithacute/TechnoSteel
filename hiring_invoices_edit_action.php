	...
	//die;
		...
		//$val['current_status'] = $invoice_result;
			...
			if ($filename != "") {
				...
				$providerid = $db->selectQuery("select `provider_id` from `sm_hiring_requirment_invoices` where `provider_id`='$id'");
				//echo "select `provider_id` from `sm_hiring_requirment_invoices` where `provider_id`='$id'"; die;
				// $candidate_code = $selectCandCode[0]['candidate_code'];
