                    ...
                    <section class="tile tile-simple no-bg">
                        ...
                        <div class="tile-body streamline timeline p-0">
                            ...
                            <ul>
                                ...
                                $selToday = $db->selectQuery("SELECT * FROM `sm_notification_employee` WHERE `notification_start_date`='$today' ORDER BY `notification_id` DESC");
                                if (count($selToday)) {
                                ...
                                <?php
                                //$text_color = "text-greensea";
                                $selYesterday = $db->selectQuery("SELECT * FROM `sm_notification_employee` WHERE `notification_start_date`='$yesterday' ORDER BY `notification_id` DESC");
                                if (count($selYesterday) > 0) {
                                    for ($yn = 0; $yn < count($selYesterday); $yn++) {
                                        ...
                                        if (strpos($notfY, "Passport") !== false) {
                                            $text_color = "text-drank";
                                        }
                                ...
                                <?php
                                $selDates = $db->selectQuery("SELECT DISTINCT  `notification_start_date` FROM `sm_notification_employee` WHERE `notification_start_date`< DATE_SUB(NOW(), INTERVAL 1 DAY)");
                                //print_r($selDates);
                                //$text_color = "text-greensea";
                                if (count($selDates)) {
                                    for ($gn = 0; $gn < count($selDates); $gn++) {
                                        $sdate = $selDates[$gn]['notification_start_date'];
                                        $selData = $db->selectQuery("SELECT * FROM `sm_notification_employee` WHERE `notification_start_date`='$sdate' ORDER BY `notification_id` DESC")
                                        ?>
