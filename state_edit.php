...
?>
             <?php
                ...
                $res = $db->selectQuery("select * from sm_recruit_state where state_id='" . $_GET['state_id'] . "' ");
	...
	$values1['state_name'] = $state_name;
	$values1['country_name'] = $country_name;
	$query1 = $db->secure_update('sm_recruit_state', $values1, "  WHERE state_id = '" . $_GET['state_id'] . "'");
	if (count($query1)) {
        ...
        <div class="pagecontent">
            ...
            <div id="rootwizard" class="tab-container tab-wizard">
                <ul class="nav nav-tabs nav-justified">
                    <li><a href="#tab1" data-toggle="tab">Edit state </a></li>
                    ...
                    <div class="tab-pane" id="tab1">
						...
						<div class="form-group col-sm-5">
                                                        ...
                                                        <select name="country_name" id="country_name" class="form-control">
                                                            <?php
                                                            $sql = $db->selectQuery("SELECT * FROM sm_recruit_country");
                                                            if (count($sql)) {
                                                                for ($cun = 0; $cun < count($sql); $cun++) {
                                                                    ?>
                                                                    <option value="<?php echo $sql[$cun]['country_name']; ?>" <?php if ($country_name == $sql[$cun]['country_name']) { ?> selected=""<?php } ?>><?php echo $sql[$cun]['country_name']; ?></option>
                                                                    <?php
                                                                }
                                                            }
                                                            ?>
                            ...
                            <div class="row">
                                <div class="form-group col-md-5">
                                    ...
                                    <input type="text" name="state_name" value="<?php echo $state_name;?>" id="state_name" class="form-control" required>
                                    <span id="com_status" class="validate_span"></span>

                                </div>
                          </div>
						  <input type="hidden" name="edit_id" id="edit_id" value="<?php echo $state_id; ?>" />
						   <div class="col-md-6">
...
$('#state_name').on('blur', function (e) {
        var state_name = jQuery('#state_name').val();
		var edit_id = jQuery('#edit_id').val();
        $('#com_status').html('Please wait...');
        $.ajax({
            url: 'state_edit_check.php',
            type: 'POST',
            dataType: 'json',
            data: {state_name: state_name,edit_id:edit_id},
