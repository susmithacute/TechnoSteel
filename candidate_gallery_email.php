...
if (isset($_POST['imgVal'])) {
    ...
    $select_cnd_data = $db->selectQuery("SELECT * FROM `sm_candidate` WHERE `candidate_id`='$candidate_id'");
    if (count($select_cnd_data) > 0) {
        $candidate_name = $select_cnd_data[0]['candidate_firstname'] . " " . $select_emp_data[0]['candidate_middlename'] . " " . $select_cnd_data[0]['candidate_lastname'];
        $message = "<table>"
                . "<tr>"
                . "<td><b>candidate Name</b>: " . $candidate_name . "</td>"
                . "</tr>"
                . "<tr>"
