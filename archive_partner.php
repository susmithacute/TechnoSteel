...
$resFet = $db->selectQuery("SELECT  `id`, `image`, `parid`, `compname`, `parname`, `phone`, `email` FROM `partner` where `custid`='" . $_SESSION['id'] . "' AND `status`='0'");
if (count($resFet)) {
...
<script type="text/javascript">
    ...
    $('#sub_btn').click(function () {
        ...
        $.ajax({
            url: "restore_part.php",
            type: "POST",
            data: {pass_val: pass_val},
            success: function (data) {
                window.location = "archive_partner.php";
            }
        });
    });</script>
