...
?>
             <?php
                ...
                $res = $db->selectQuery("select * from sm_job_category where job_category_id='" . $_GET['job_category_id'] . "' ");


                if (count($res)) {

                    $job_category_id = $res[0]['job_category_id'];
					$job_category_name = $res[0]['job_category_name'];
					$job_position = $res[0]['job_position'];
	...
	$values1['job_category_name'] = $job_category_name;
	$values1['job_position'] = $job_position;
	$query1 = $db->secure_update('sm_job_category', $values1, "  WHERE job_category_id = '" . $_GET['job_category_id'] . "'");
	if (count($query1)) {
        ...
        <div class="pagecontent">
            ...
            <div id="rootwizard" class="tab-container tab-wizard">
                <ul class="nav nav-tabs nav-justified">
                    <li><a href="#tab1" data-toggle="tab">Edit Category </a></li>
						...
						<div class="form-group col-sm-5">
                                                        ...
                                                        <select name="job_position" id="job_position" class="form-control">
                                                            ...
                                                            if (count($sql)) {
                                                                for ($cun = 0; $cun < count($sql); $cun++) {
                                                                    ?>
                                                                    <option value="<?php echo $sql[$cun]['job_name']; ?>" <?php if ($job_position == $sql[$cun]['job_name']) { ?> selected=""<?php } ?>><?php echo $sql[$cun]['job_name']; ?></option>
                                                                    <?php
                                                                }
                                                            }
                                                            ?>
                            ...
                            <div class="row">
                                <div class="form-group col-md-5">
                                    ...
                                    <input type="text" name="job_category_name" value="<?php echo $job_category_name;?>" id="job_category_name" class="form-control" required>
                                    <span id="com_status" class="validate_span"></span>

                                </div>
                          </div>
						  <input type="hidden" name="edit_id" id="edit_id" value="<?php echo $job_category_id; ?>" />
						   <div class="col-md-6">
...
$('#job_category_name').on('blur', function (e) {
        ...
        $.ajax({
            ...
            type: 'POST',
            dataType: 'json',
            data: {job_category_name: job_category_name,edit_id:edit_id,job_position: job_position},
