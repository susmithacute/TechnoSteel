...
$resFet = $db->selectQuery("SELECT CONCAT( sm_candidate.candidate_firstname,  ' ', sm_candidate.candidate_middlename,  ' ',
 sm_candidate.candidate_lastname ) AS candidate_name,sm_candidate.candidate_code,sm_candidate.candidate_nationality, sm_candidate_visa_process.candidate_id,sm_candidate_visa_process.visa_type,sm_candidate_application.application_job_position,sm_visa_type.visa_type_name
FROM sm_candidate LEFT JOIN sm_candidate_visa_process ON  sm_candidate.candidate_id=sm_candidate_visa_process.candidate_id LEFT JOIN sm_candidate_application ON  sm_candidate.candidate_id=sm_candidate_application.candidate_id
LEFT JOIN sm_visa_type ON sm_candidate_visa_process.visa_type=sm_visa_type.visa_type_id WHERE  `visa_status`='Applied'");
if (count($resFet)) {
...
<div class="modal-body">
								   ...
								   <label> Date:</label>
								    <div class='input-group datepicker' data-format="L">
                                        <input type='text' name="doj" id="todo_edate3" class="form-control"    />
                                        <span class="input-group-addon"> <span class="fa fa-calendar"></span> </span>
                                    </div>
							</div>
				...
				<div id="SucM" style="margin-left:40px; color:red;"></div>
...
<script>
$('#todo_edate').datepicker({dateFormat: 'dd/mm/yy', changeMonth: true, changeYear: true,onSelect: function(date) {
$("#todo_edate1").datepicker('option', 'minDate', date);} });
$('#todo_edate1').datepicker({dateFormat: 'dd/mm/yy', changeMonth: true, changeYear: true});
			...
			{"data": "candidate_code"},
               ...
               
                {
                    ...
                    "render": function (data) {
                        return '<a onclick="update_id(' + data + ')" class="btn btn-xs btn-green" data-toggle="modal" data-target="#splash1" data-options="splash-ef-3"> Visa Details</a>&nbsp<a onclick="edit_id(' + data + ')" class="btn btn-xs btn-default mr-5" data-toggle="modal" data-target="#splash" data-options="splash-ef-3"><i class="fa fa-pencil"></i>Edit</a><a onclick="delete_id(' + data + ')" class="btn btn-xs btn-blue" data-toggle="modal" data-target="#splash2" data-options="splash-ef-3"></i>Cancel</a>';
                    }}
            ],
            "aoColumnDefs": [
...
<script type="text/javascript">
	...
	<!---- Upload Visa Details --->
					...
					//alert(candidate_id);
					var visa_status=$('#medical_result').val();					//alert(visa_status);
							...
							if(visa_no==''||visa_issued==''||visa_expiry=='')
								{
									...
									$('#error').html("enter all fields");
											...
											type: "POST",
											data: {candidate_id:candidate_id,visa_no:visa_no,visa_issued:visa_issued,visa_expiry:visa_expiry,visa_status:visa_status,reason:reason},
											success: function (data) {
												//alert(data);
												
												if(data == 0){
	  $('#SucM').html("Visa No is Already Exist").show(30000);
}
else if(data == 1){
                    //setTimeout('Redirect()', 20000);

 $('#SucM').html("Success").show(30000);
...
<script>
	...
	function edit_id(id)
    {
		...
		$.ajax({
			url:"visa_applied_edit.php",
			dataType:"json",
			method:"POST",
			data:{edit_id:id},
			success:function(data){
				//alert(data);
				 $('#todo_edate3').val(data.applied_visa_date);
				 $('#selected_visa_type').val(data.visa_type);
       ...
       $('#edit_id').val($.session.get('edit_seesion'));
		...
		var applied_visa_date=$('#todo_edate3').val();
		var visa_type=$('#selected_visa_type').val();
		var visa_category=$('#visa_category').val();
       //alert(candidate_id);
	  
	  if(applied_visa_date==''||visa_type=='Select'||visa_category=='Select')
								{
        ...
        $.ajax({
            url: "visa_applied_edit_action.php",
            type: "POST",
            data: {applied_visa_date:applied_visa_date,visa_type:visa_type,visa_category:visa_category,candidate_id:candidate_id},
            success: function (data) {
