...
AS employee_name,sm_employee.employee_employment_id,sm_employee.employee_id,sm_employee_performance.rating1,sm_employee_performance.date,sm_employee_performance.id,sm_employee_performance.employee_id,sm_rating.rating FROM sm_employee  LEFT JOIN sm_employee_performance ON sm_employee.employee_id=sm_employee_performance.employee_id LEFT JOIN sm_rating ON sm_employee_performance.rating1=sm_rating.id  WHERE `status`='active' AND sm_employee_performance.employee_id='$id'");
...
$name = $values['employee_name'];
	   ...
	   $values['rating1'] = $resFet[$rC]['rating1'];
	    $values['rating'] = $resFet[$rC]['rating'];
	 $var = $resFet[$rC]['date']."-01";
	$date = date('Y M', strtotime(str_replace('-', '/', $var)));
        ...
        <div class="pagecontent">
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    ...
                    <!-- tile -->
                    <section class="tile">
<?php $emp_names = $db->selectQuery("SELECT CONCAT( sm_employee.employee_firstname,  ' ', sm_employee.employee_middlename,  ' ', sm_employee.employee_lastname ) AS employee_name,employee_employment_id FROM sm_employee WHERE `employee_id`='$id'");
					?>
					
                        <!-- tile header -->
                        <div class="tile-header dvd dvd-btm">
                            <h1 class="custom-font"><strong>Employee Performance List </strong>&nbsp&nbsp&nbsp&nbsp<span><font size="4"><?php echo $emp_names[0]['employee_name']; ?></font></span>&nbsp<i><span><font size="3"><strong><?php echo "	"."	".$emp_names[0]['employee_employment_id'];?></strong></i></span></span></h1>
                            <ul class="controls">
							<li>
 <a href="employee_perform.php" role="button" tabindex="0" class="tile-add" title="add">
...
<script type="text/javascript">
	...
	function delete_id(id)
    {
        ...
        $('#id').val($.session.get('delete_seesion'));
	 ...
	 $('#sub_btn').click(function () {
        ...
        $.ajax({
            url: "perform_delete.php",
            type: "POST",
            data: {pass_val: pass_val},
            success: function (data) {
				//alert(data);
               window.location = 'employee_perform_list.php?employee_id=<?php echo $id ?>';
            }
        });
    });
