...
<section id="content">
    <div class="page page-timeline">
        <div class="pageheader">
            <h2>Visa Validity Expiry<span>Notifications</span></h2>
            <div class="page-bar">
                    ...
                    <section class="tile tile-simple no-bg">
                        ...
                        <div class="tile-body streamline timeline p-0">
                            ...
                            <ul>
                                ...
                                $selToday = $db->selectQuery("SELECT * FROM `sm_notification_visa` WHERE `notification_start_date`='$today' ORDER BY `notification_id` DESC");
                                if (count($selToday)) {
                               ...
                               <li class="heading"><a href="javascript:;" class="btn btn-lightred b-0">Yesterday</a></li>
                                <?php
                                //$text_color = "text-greensea";
                                $selYesterday = $db->selectQuery("SELECT * FROM `sm_notification_visa` WHERE `notification_start_date`='$yesterday' ORDER BY `notification_id` DESC");
                                ...
                                <?php
                                $selDates = $db->selectQuery("SELECT DISTINCT  `notification_start_date` FROM `sm_notification_visa` WHERE (`notification_start_date`< DATE_SUB(NOW(), INTERVAL 1 DAY))&&(`notification_start_date`!='$yesterday')");
                                //print_r($selDates);
                                //$text_color = "text-greensea";
                                if (count($selDates)) {
                                    for ($gn = 0; $gn < count($selDates); $gn++) {
                                        $sdate = $selDates[$gn]['notification_start_date'];
                                        $selData = $db->selectQuery("SELECT * FROM `sm_notification_visa` WHERE `notification_start_date`='$sdate' ORDER BY `notification_id` DESC")
                                        ?>
