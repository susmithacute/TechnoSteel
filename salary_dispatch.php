...
$y = date("Y");
$cmpArray = array();
$resFet = $db->selectQuery("SELECT CONCAT( sm_employee.employee_firstname, ' ', sm_employee.employee_middlename, ' ', sm_employee.employee_lastname ) AS full_name, sm_employee.employee_id, sm_employee.employee_designation, sm_payroll.payroll_date, sm_payroll.payroll_id, sm_payroll.salary,sm_payroll.company_id, sm_payroll.payroll_status
FROM sm_payroll
INNER JOIN sm_employee ON sm_payroll.employee_id = sm_employee.employee_id
WHERE sm_payroll.payroll_status = 'Not Paid'
AND MONTH( sm_payroll.payroll_date ) = '$m'
AND YEAR( sm_payroll.payroll_date ) = '$y'
AND sm_employee.employee_status='1'");
if (count($resFet)) {
    for ($rC = 0; $rC < count($resFet); $rC++) {
        echo $company_id = $resFet[$rC]['company_id'];
...
<section id="content">
    <div class="page page-shop-products">
        <div class="pageheader">
            <h2>Salary Dispatch<span></span></h2>
            <div class="page-bar">
                ...
                <div class="col-md-12">
                    <section class="tile">
                        <div class="tile-header dvd dvd-btm">
                            <h1 class="custom-font"><strong>Salary</strong> Dispatch</h1>
                            <ul class="controls">
                                <li class="dropdown">
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <span id="date_span" class="validate_span"></span>
                    </div>
                </div>
            </div>
...
<div class="modal splash fade" id="splash1" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h3 class="modal-title custom-font">Edit salary of this month?</h3>
            </div>
            <input type="hidden" id="hid_del" value=""/>
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control" value="<?php echo $salary;?>" name="emp_salary" id="emp_salary" />
                        <span id="salary_span" class="validate_span"></span>
                    </div>
                </div>
            </div>
            <div class="modal-footer">
            ...
            "columns": [
                ...
                {"data": "company_name"},
                {
                    ...
                    "render": function (data) {
                        return '<a onclick="delete_id(' + data + ')" class="btn btn-xs btn-green" data-toggle="modal" data-target="#splash" data-options="splash-ef-3"><i class="fa fa-check"></i> Pay</a> <a onclick="delete_id(' + data + ')" class="btn btn-xs btn-red" data-toggle="modal" data-target="#splash1" data-options="splash-ef-3"><i class="fa fa-pencil"></i> Edit</a>';
                    }}
            ],
            "aoColumnDefs": [
...
<script type="text/javascript">
    ...
    $('#sub_btn').click(function () {
        ...
        if(salary_date==""){
            $('#date_span').html("This value is required.");
        }
        else{
        $.ajax({
            ...
            success: function (data) {
                window.location = "salary_dispatch.php";
            }
        });
        }
    });
    ...
    $('#update_btn').click(function () {
        var pass_val = $('#hid_del').val();
        var up_salary = $('#emp_salary').val();
        if (up_salary == "") {
            $('#salary_span').html("This value is required.");
        }
        else{
        $.ajax({
            url: "payroll_actions_employee.php",
            type: "POST",
            data: {cancel_id: pass_val, up_salary: up_salary},
            success: function (data) {
                window.location = "salary_dispatch.php";
            }
        });
