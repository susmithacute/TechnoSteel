...
$category_list = $db->selectQuery("SELECT  `job_category_id`, `job_category_name`,`job_position` FROM `sm_job_category` where `job_category_status`='1' ORDER BY job_category_id DESC ");
if (count($category_list )) {
    for ($cou = 0; $cou < count($category_list ); $cou++) 
	{
		$sl=$cou+1;
	    $values['sl_id']=$sl;
        $values['job_category_id'] = $category_list[$cou]['job_category_id'];
        $values['job_category_name'] = $category_list[$cou]['job_category_name'];
		$values['job_position'] = $category_list[$cou]['job_position'];
		//$values['company_status'] = $resFet[$rC]['company_status'];
        $coufArray["data"][] = $values;
    }
}
$fp = fopen('assets/extras/categ.json', 'w');
                ...
                <div class="col-md-12">
                    ...
                    <section class="tile">
                        ...
                        <div class="tile-header dvd dvd-btm">
                            <h1 class="custom-font"><strong> </strong>Category List</h1>
                            <ul class="controls">
            ...
            "columns": [
               ...
               
                {
                    ...
                    "render": function (data) {
                        return '<a href="category_edit.php?job_category_id=' + data + '" class="btn btn-xs btn-default mr-5"><i class="fa fa-pencil"></i> Edit</a><a onclick="delete_id(' + data + ')" class="btn btn-xs btn-lightred" data-toggle="modal" data-target="#splash" data-options="splash-ef-3"><i class="fa fa-times"></i> Delete</a>';
                    }}
            ],
            "aoColumnDefs": [
