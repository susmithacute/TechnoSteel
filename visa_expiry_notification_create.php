...
$cur_date = date("d-m-Y");
$notif_day_before=45;
$select_day=$db->selectQuery("SELECT `notify_period` FROM `sm_visa_validity` WHERE `notification_name`='Visa notification'");
if(count($select_day)){
             ...
             for ($d = 0; $d < count($doc_n); $d++) {
                ...
                if ($num_days < $notif_day_before && $num_days >= 0) {
					...
					//echo "<pre>";print_r($values);die;
					    $check_exist = $db->selectQuery("SELECT * FROM `sm_notification_visa` WHERE `cand_id` = '$canId' AND `visa_name` = '$visa_name'");
                if (count($check_exist) > 0) {
                    for ($fc = 0; $fc < count($check_exist); $fc++) {
                        ...
                        $check_id = $check_exist[$fc]['cand_id'];
                            ...
                            $db->secure_update("sm_notification_visa", $values_up, "WHERE `cand_id`='$check_id'");
                    ...
                    try {
                        $in = $db->secure_insert("sm_notification_visa", $values);
                        if (empty($in)) {
