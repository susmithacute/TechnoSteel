...
$cmpArray = array();
$resFet = $db->selectQuery("SELECT sm_visa_renew.renew_id, sm_visa.visa_applicant_name,
    sm_visa_renew.visa_id,  sm_visa.visa_sponsor,
   sm_visa_category.visa_category, sm_visa_renew.visa_renewal_amount,
    sm_visa_renew.visa_renewal_type, sm_visa.visa_type, sm_visa_renew.visa_bank_fee,
    sm_visa_renew.visa_sponsor_fee, sm_visa_renew.visa_company_fee, sm_visa_renew.visa_total_fee,
    sm_visa_renew.visa_balance_fee, sm_visa_renew.visa_advance_fee,sm_visa_type.visa_type_name,sm_visa_renew.visa_renewal_inserted_date,sm_visa.visa_client_name
FROM sm_visa_renew
INNER JOIN sm_visa ON sm_visa_renew.visa_id = sm_visa.visa_id
INNER JOIN sm_visa_type ON sm_visa.visa_type=sm_visa_type.visa_type_id
INNER JOIN sm_visa_category ON sm_visa_category.visa_category_id=sm_visa_renew.visa_renewal_type
WHERE visa_active =1");
if (count($resFet)) {
    for ($rC = 0; $rC < count($resFet); $rC++) {
        $visa_client_id = $resFet[$rC]['visa_client_name'];
        $visa_client = $db->selectQuery("SELECT * FROM `sm_visa_client` WHERE `visa_client_id`='$visa_client_id'");
        if (count($visa_client) > 0) {
            $visa_client_name = $visa_client[0]['visa_client_name'];
        }
        $payment_date = new DateTime($resFet[$rC]['visa_renewal_inserted_date']);
        $payment_month = $payment_date->format("F");
        $payment_year = $payment_date->format("Y");
        $values['payment_month'] = $payment_month;
        $values['payment_year'] = $payment_year;
                                ...
                                <table class="table table-striped table-hover table-custom" id="payment_list">
                                    ...
                                    <thead>
                                        <tr>
                                            ...
                                            <th style="width:80px;">Payment Month</th>
                                            <th style="width:80px;">Payment Year</th>
...
//                }, 0);
                ...
                total_payment = api.column(7, {page: 'current'}).data().reduce(function (a, b) {
                    return intVal(a) + intVal(b);
                }, 0);
                total_bank = api.column(8, {page: 'current'}).data().reduce(function (a, b) {
                    return intVal(a) + intVal(b);
                }, 0);
                total_sponsor_fee = api.column(9, {page: 'current'}).data().reduce(function (a, b) {
                    return intVal(a) + intVal(b);
                }, 0);
                total_company = api.column(10, {page: 'current'}).data().reduce(function (a, b) {
                    return intVal(a) + intVal(b);
                }, 0);
                total_advance = api.column(11, {page: 'current'}).data().reduce(function (a, b) {
                    return intVal(a) + intVal(b);
                }, 0);
                total_balance = api.column(12, {page: 'current'}).data().reduce(function (a, b) {
                    return intVal(a) + intVal(b);
                }, 0);
