...
if(isset($_POST['designation'])){
  ...
  $des_names = $db->selectQuery("SELECT `designation_name` FROM `sm_designation` WHERE `designation_id` = '$designation'");
   if (count($des_names) > 0)
    ...
    $employee_name=$db->selectQuery("SELECT CONCAT(sm_employee.employee_firstname,' ',sm_employee.employee_middlename,' ',sm_employee.employee_lastname)as fullname,employee_id,employee_employment_id,employee_company FROM `sm_employee` WHERE  `employee_designation`='$des_name'");
    if(count($employee_name)){ ?>
		<div class="form-group col-md-2">
                                    ...
                                    <label for="phone">Employee ID: </label>
									 ...
									 <option value="" selected="">Select</option>
									<?php for($ds=0; $ds < count($employee_name); $ds++){ ?>
									<option value="<?php echo $employee_name[$ds]['employee_id'];?>"><?php echo $employee_name[$ds]['employee_employment_id'];?></option>
									<?php } ?>
									</select>
                                    </div>
       <?php } else { ?>
