<?php
$page = "notification";
$sub="notification_vehicle";
$sub1="notification_vehicle_regis";
include ("./file_parts/header.php");
?>
<section id="content">
    <div class="page page-timeline">
        <div class="pageheader">
            <h2>Vehicle <span>Notifications</span></h2>
            <div class="page-bar">
                    ...
                    <section class="tile tile-simple no-bg">
                        ...
                        <div class="tile-body streamline timeline p-0">
                            ...
                            <ul>
                                ...
                                $selToday = $db->selectQuery("SELECT * FROM `sm_vehicle_notification` WHERE `notification_start_date`='$today' AND `notification_title`='Registration' ORDER BY `notification_id` DESC");
                                if (count($selToday)) {
                                    for ($tn = 0; $tn < count($selToday); $tn++) {
                                        $notfT = $selToday[$tn]['notification_data'];
                                        if (strpos($notfT, "Registration") !== false) {
                                ...
                                <?php
                                //$text_color = "text-greensea";
                                $selYesterday = $db->selectQuery("SELECT * FROM `sm_vehicle_notification` WHERE `notification_start_date`='$yesterday' AND `notification_title`='Registration' ORDER BY `notification_id` DESC");
                                ...
                                <?php
                                $selDates = $db->selectQuery("SELECT DISTINCT  `notification_start_date` FROM `sm_vehicle_notification` WHERE `notification_title`='Registration' AND `notification_start_date`< DATE_SUB(NOW(), INTERVAL 1 DAY)");
                                $text_color = "text-greensea";
                                if (count($selDates)) {
                                    for ($gn = 0; $gn < count($selDates); $gn++) {
                                        $sdate = $selDates[$gn]['notification_start_date'];
                                        $selData = $db->selectQuery("SELECT * FROM `sm_vehicle_notification` WHERE `notification_title`='Registration' AND `notification_start_date`='$sdate' ORDER BY `notification_id` DESC")
                                        ?>
